...
final class ErrorHandler implements Handler {
	...
	public static function logException(
		...
		// Retrieves the requested URL from $_SERVER
		if (strcmp(self::getServerVariable("HTTP_REFERER", "%%WRONG_URL%%"), "%%WRONG_URL%%") == 0) {
			$url = self::getServerVariable("REQUEST_SCHEME", "https") . "://" .
					self::getServerVariable("HTTP_HOST", DOMAIN) .
			...
			$url = self::getServerVariable("HTTP_REFERER", "Unknown URL");
		}
		...
		Utils::sendMail(
				APPLICATION_NAME . " - Erreur PHP le " . Utils::formatDate(time(), true),
				$dump,
				MAIL_DEVELOPER
		);
	}
	...
	private static function dumpException(\Throwable $throwable) {
		...
		<html>
			...
			<body>
				<h1><?= self::getPhpErrorFromNumber($throwable->getCode()); ?> >> Uncaught <?= get_class($throwable); ?></h1>
	...
	private static function getServerVariable(
		...
		return getValueInArrayOrDefault($varName, $_SERVER, $defValue);
	}
