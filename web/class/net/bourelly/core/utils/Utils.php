...
final class Utils {
	 ...
	 */
	public static final function callStack(bool $debugInFile = true): string {
	 ...
	 */
	public static final function log(string $message, int $time = NULL): void {
	...
	/**
	 * Translate the formatted date (DD/MM/YYYY) into unix timestamp.
	 *
	 * @param string $date the date in format (DD/MM/YYYY)
	 * @return int the timestamp unix
	 */
	public static final function dateTimestampUnix(string $date): int {
	...
	public static final function sendMail(
		...
		$headers .= "From: " . APPLICATION_NAME . " <" . $mailFrom . ">\n";
		if (!is_null($mailCopy)) {
			$headers .= "Cc: " . $mailCopy . "\n";
		}
	 ...
	 * @see header()
	 */
	public static final function redirect(string $uri): void {
		header("Location: " . $uri);
		session_write_close();
		exit;
	}
	 ...
	 */
	public static final function plural(int $nb, string $word): string {
		if ($nb > 1) {
	 ...
	 */
	public static final function sha512(string $string, string $salt = NULL): string {
		$hash = !is_null($salt) ?
	 ...
	 */
	public static final function htmlentities_array_map($toEncode): string {
	 ...
	 */
	public static final function html_entity_decode_array_map($toDecode): string {
